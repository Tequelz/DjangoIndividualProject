...
class LessonIDCheck(generics.ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        qs = list(Lecture.objects.filter(lec_id= request.data.get("lec_id"),lec_number= request.data.get("lec_num"),lec_length=request.data.get("lec_len")).values_list('pk',flat=True))
